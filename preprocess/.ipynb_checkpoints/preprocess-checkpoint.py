    ...
    for idx, (src, trgs) in enumerate(tqdm(src_trgs_pairs)):
        ...
        for trg in trgs:
            ...
            if (
                ...
                and (
                    len(trg_tokens) > 0
                    and re.match(r"\d\d[a-zA-Z\-]\d\d", trg_tokens[0].strip())
                )
                or (
                    len(trg_tokens) > 1
                    and re.match(r"\d\d\w\d\d", trg_tokens[1].strip())
                )
            ):
                continue
...
def main_preprocess(opt, input_mode, save_mode):
    ...
    logger.info(
        "success loaded %s %s data : %d "
        % (opt.dataset_class, input_mode, len(source_data))
    )
    ...
    if opt.dataset_class == "openkp" and input_mode == "EvalPublic":
        ...
        logger.info(
            "success obtain %s present keyphrase : %d (filter out : %d)"
            % (input_mode, len(feed_data), (len(refactor_data) - len(feed_data)))
        )

    # save ground-truth
    gt_filename = os.path.join(
        opt.output_path, "%s.%s_candidate.json" % (opt.dataset_class, save_mode)
    )
    ...
    save_filename = os.path.join(
        opt.output_path, "%s.%s.json" % (opt.dataset_class, save_mode)
    )
    save_preprocess_data(feed_data, save_filename)
    logger.info(
        "success saved %s %s data: %d to %s"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="preprocess_for_kpe.py",
    ...
    if not os.path.exists(opt.output_path):
        ...
        logger.info(
            "don't exist this output dir, remake a new to : %s" % opt.output_path
        )
