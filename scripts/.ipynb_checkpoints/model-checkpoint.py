...
class KeyphraseSpanExtraction(object):
    ...
    def init_optimizer(self, num_total_steps):
        ...
        self.optimizer = AdamW(
            optimizer_grouped_parameters, lr=self.args.learning_rate, correct_bias=False
        )
        ...
        #     self.optimizer, num_warmup_steps=num_warmup_steps, num_training_steps=-1
        # )
    ...
    def load_checkpoint(filename, new_args=None):
        ...
        logger.info(
            "success loaded epoch_%d checkpoints ! From : %s" % (epoch, filename)
        )
        return model, epoch
